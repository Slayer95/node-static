...
Server.prototype.servePath = function (pathname, status, headers, req, res, pathValidator, finish) {
    var that = this,
    ...
    if (pathname.indexOf(that.root) === 0) {
        if (pathValidator && !pathValidator(pathname)) {
            // Forbidden
            finish(403, {});
            return promise;
        }
...
Server.prototype.serve = function (req, res, options, callback) {
    ...
    process.nextTick(function () {
        that.servePath(pathname, 200, {}, req, res, options && options.validatePath, finish).on('success', function (result) {
            promise.emit('success', result);
...
Server.prototype.respondNoGzip = function (pathname, status, contentType, _headers, files, stat, req, res, finish) {
    ...
    if (files.length == 1 && byteRange.valid) {
        if (byteRange.to < length) {
            ...
            console.warn("Range request exceeds file '" + key + "' boundaries, goes until byte no", byteRange.to, "against file size of", length, "bytes");
        }
    }
...
Server.prototype.respond = function (pathname, status, _headers, files, stat, req, res, finish) {
    ...
    if (!contentType) {
        var autoMime = mime.getType(files[0]);
        if (autoMime && this.options.charset && autoMime.startsWith('text/')) {
            contentType = autoMime + '; charset=' + this.options.charset;
        } else if (autoMime) {
            contentType = autoMime;
        } else {
